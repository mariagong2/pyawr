...
class class_TDR_functions():
    ...
    def Truncate_TDR_Respone(self, TDR_Data_ay):
        ...
        #
        NumPts = len(TDR_Data_ay)
        FirstIndex = int(-self.MinTimeBin)
        LastIndex = int(NumPts)
        if len(TDR_Data_ay.shape) == 2:
            NumCols = len(TDR_Data_ay[0])
    ...
    def GateTimeCalc(self, MarkerTime_list, TDR_Time_ay, StructureDelay, NumPorts):#---------------------------------
        ...
        #
        Gate_Start_time_11 = MarkerTime_list[0]
        Gate_Stop_time_11 = MarkerTime_list[1]
    ...
    def Filter_TDR_Response(self, TDR_LPI_Data_ay, GateTimeData_list, TDR_Filter_Params):#------------------------------------
        ...
        #
        NumTraces = len(TDR_LPI_Data_ay[0])-1 #First col is Time data, does not count
        NumTDR_Pts = len(TDR_LPI_Data_ay[:,0])
    ...
    def Calc_Sparam_FFT(self, TDR_Data_ay, FFT_WindowType):#------------------------------------
        ...
        for i in range(NumSparams):
            TimeData_ay = TDR_Data_ay[:,i+1]
            fft_v = np.fft.fft(TimeData_ay)*(2/N) #Real Data, so scale by 2/N
            fft_v *= self.Num_FreqSamplePts #Scaling: Original un-mirrored freq data has M points
            LastIndex = int(self.Num_FreqSamplePts)
            Truncated_fft_v = fft_v[0:LastIndex]# Remove mirrored neg freq section and zero padding
            Scaled_fft_v = self.UndoWindowingFromFreqData(Truncated_fft_v, FFT_WindowType)
            Repaired_fft_v = self.ExtrapolateLastPoint(Scaled_fft_v, FFT_WindowType) #Last point may be corrupt
            SparamData_Complex_ay[:,i] = Repaired_fft_v
        #end for
...
#end if
#
GateTimeData_list = tdr.GateTimeCalc(LPS_Markers_ay, TDR_Time_ay, StructureDelay, NumPorts)
Filtered_TDR_ay = tdr.Filter_TDR_Response(TDR_LPI_Data_ay, GateTimeData_list, TDR_Filter_Params)
MaxFreq = DF_Freq_ay[1]
